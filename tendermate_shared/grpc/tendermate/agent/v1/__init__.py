...
@dataclass(eq=False, repr=False)
class ProductStarted(betterproto.Message):
    """
...
@dataclass(eq=False, repr=False)
class SearchEvent(betterproto.Message):
    """
    A single event on the SearchProducts stream: exactly one of the variants.
    """

    started: "ProductStarted" = betterproto.message_field(1, group="event")
    result: "MatchResult" = betterproto.message_field(2, group="event")
...
class SearchServiceBase(ServiceBase):
    ...
    async def __rpc_search_products(
        self, stream: "grpclib.server.Stream[SearchRequest, SearchEvent]"
    ) -> None:
